...
function embedText(text='', dim=128){
  ...
  const toks=String(text).toLowerCase().replace(/https?:\/\/\S+/g,' ').replace(/[^\p{L}\p{N}.\s-]/gu,' ').split(/\s+/).filter(w=>w.length>2)
  for(const t of toks){ const h=hashToken(t); const idx=h%dim; v[idx]+=((h&1)?1:-1)*(1+Math.min(3,t.length/8)) }
...
function saveChunkVector(chunkId, text){ db.prepare(`INSERT INTO rag_evidence_vectors (chunk_id,vector_json,dim,updated_at) VALUES (?,?,128,datetime('now')) ON CONFLICT(chunk_id) DO UPDATE SET vector_json=excluded.vector_json, updated_at=datetime('now')`).run(chunkId, JSON.stringify(embedText(text))) }
export function ragSemanticSearch(query,{limit=8, minScore=0.08}={}){
...
export function upsertRagDocument({url,title,source='',publishedAt='',content='',assetTags=[]}){
  ...
  const fts=db.prepare(`INSERT INTO rag_evidence_chunks_fts (rowid,title,source,content,asset_tags) VALUES ((SELECT rowid FROM rag_evidence_chunks WHERE id=?),?,?,?,?)`)
  chunkText(content).forEach((chunk,i)=>{ const cid=hash(`${id}:${i}:${chunk}`).slice(0,32); insert.run(cid,id,i,clean(title),source||host(url),url,chunk,tags,fetchedAt); fts.run(cid,clean(title),source||host(url),chunk,tags); saveChunkVector(cid, `${title} ${chunk} ${tags}`) })
  ...
  return {id,url,title:clean(title),chunks:chunkText(content).length}
}

function scoreSource(source,publishedAt){ let s=50; if(/idx|bi\.go|ojk|kontan|bisnis|cnbc|reuters|bloomberg|yahoo/i.test(source)) s+=25; const age=publishedAt?Date.now()-Date.parse(publishedAt):0; if(age && age<36e5*24) s+=15; if(age && age>36e5*24*7) s-=20; return Math.max(0,Math.min(100,s)) }

export function ingestTopicsToRag(topics=[]){
  initRagSchema(); const out=[]
  for(const topic of topics){ for(const item of topic.items||[]){ if(!item?.url && !item?.link) continue; const url=item.url||item.link; const content=[item.title,item.snippet,item.summary,topic.intro].filter(Boolean).join('\n\n'); const r=upsertRagDocument({url,title:item.title,source:item.source||host(url),publishedAt:item.createdAt||item.created_at||'',content,assetTags:inferTags(`${item.title} ${item.snippet||''} ${topic.title||''}`)}); if(r) out.push(r) } }
  return out
}
...
export function ragSearch(query,{section='',limit=8}={}){
  ...
  try { rows=db.prepare(`SELECT c.id chunk_id,c.document_id,c.title,c.source,c.url,c.content,c.asset_tags,bm25(rag_evidence_chunks_fts) rank FROM rag_evidence_chunks_fts JOIN rag_evidence_chunks c ON rag_evidence_chunks_fts.rowid=c.rowid WHERE rag_evidence_chunks_fts MATCH ? ORDER BY rank LIMIT ?`).all(ftsQuery,limit*3) } catch { rows=db.prepare(`SELECT id chunk_id,document_id,title,source,url,content,asset_tags,0 rank FROM rag_evidence_chunks WHERE lower(title || ' ' || content) LIKE ? LIMIT ?`).all(`%${terms[0]||q.toLowerCase()}%`,limit*3) }
  const seen=new Set(); rows=rows.filter(r=>{ const k=r.url; if(seen.has(k)) return false; seen.add(k); return true }).filter(r=>{
    // Filter out test data: fake domains, test entries
...
export function buildRagContext(topics=[]){
  ...
  const sections=queries.map(([section,q])=>({section,query:q,results:ragSearch(q,{section,limit:5})}))
...
export function saveRagCitations(reportSlug, rag){
  ...
  let n=0; for(const c of rag.citations){ stmt.run(reportSlug,c.section,c.document_id,c.chunk_id,c.title,c.url,c.source||host(c.url)); n++ }
  db.prepare(`INSERT INTO rag_evidence_quality_scores (report_slug,citation_coverage,source_diversity,duplicate_clusters,freshness_score,unsupported_claims) VALUES (?,?,?,?,?,?)`).run(reportSlug,rag.quality.coverage,rag.quality.sourceDiversity,0,rag.quality.coverage,0)
  return n
}
...
export function vectorizeMissingChunks({limit=100}={}){
  initRagSchema()
  ...
  for(const r of rows) saveChunkVector(r.id, `${r.title} ${r.content} ${r.asset_tags||''}`)
  return { ok:true, requested:limit, vectorized:rows.length, stats:ragStorageStats() }
}
