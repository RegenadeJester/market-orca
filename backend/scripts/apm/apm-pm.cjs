...
function scanMissingCatches(dir, label) {
  ...
  for (const file of files) {
    ...
    for (let i = 0; i < lines.length; i++) {
      // Skip Vue template lines (@click="fn", :class= etc.) and non-code
      if (/^\s*<|^\s*@|^\s*v-|^\s*:class|^\s*:style|\)\s*$/.test(lines[i])) continue
      if (/\.then\(/.test(lines[i])) {
        // Vue `.then` in template context - skip
        if (lines[i].includes('v-if') || lines[i].includes('@click') || lines[i].includes('v-for')) continue
        // Look ahead max 5 lines for .catch(
        const following = lines.slice(i, i + 5).join('\n');
...
// ── Deduplicate pain points (same file + type + similar line) ──
function deduplicate(points) {
  const seen = new Set()
  const unique = []
  for (const p of points) {
    const key = `${p.type}:${p.file}:${Math.floor((p.line - 1) / 5) * 5}` // group within 5 lines
    if (!seen.has(key)) {
      seen.add(key)
      unique.push(p)
    }
...
// ── Rank & pick top 3 ──
const deduped = deduplicate(painPoints)
...
> Generated by PM Agent scan at 08:00 WIB from ${deduped.length} identified pain points
...
## Full Scan Results (${deduped.length} total)
...
- **Total pain points:** ${deduped.length}
...
console.log(`   (Total: ${deduped.length} pain points after dedup, P1: ${ranked.filter(p => p.priority === 'P1').length})`);
